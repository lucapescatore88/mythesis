...
\centering 
\unitlength = 1mm
\begin{fmffile}{penguinNP_diagram}
 \subfloat{
	...
	\fmfiequ{em}{ep+(.7w,0)}
	\fmfi{photon,lab=$W$,lab.sid=left}{Vtb--Vts}
	...
	\fmfi{photon}{p--p'}
	\fmfi{fermion,lab=$\ell^+$,lab.sid=left}{ep--p'}
	\fmfi{fermion,lab=$\ell^-$,lab.sid=left}{p'--em}
...
\hspace{1cm}
 \subfloat{
	...
	\fmfiequ{.5[ep,em]}{.5[sw,se]}
	\fmfiequ{em}{ep+(.7w,0)}
	\fmfi{boson,lab=new,lab.sid=left}{Vtb--Vts}
	\fmfi{fermion,lab=$b$,lab.sid=left}{b--Vtb}
	\fmfi{fermion,lab=$s$,lab.sid=left}{Vts--s}
	\fmfi{fermion,lab=$t$}{Vtb{b-Vtb} .. tension 2 .. {right}p}
	\fmfi{fermion,lab=$t$}{p{right} .. tension 2 .. {Vts-s}Vts}
...
1~\gevgevcccc~the photon pole dominates leaving little prospect for new physics to be found. %~\ref{sec:theo_qsq}.
...
%between data and re-weighted Monte Carlo and show that a good agreement is achieved.
