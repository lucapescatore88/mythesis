...
are an ideal laboratory to study LFU as new physics contributing in the loops could break the flavour symmetry.
...
\end{equation}
where $H$ can be an inclusive state containing 
an $s$ quark ($X_s$) or an s-quark resonance like $K$ or $\Kstarz$.
In this quantity the differential branching fraction is integrated over the squared dilepton invariant mass, \qsq, from 
...
and $R_K$ has been recently measured also at LHCb~\cite{Aaij:2014ora}, which represents the most 
precise measurement to date; measured values are summarised in Tab.~\ref{tab:expstatus}.
...
ratio which probes LFU and was measured to be $2.1~\sigma$ larger than the value expected from the assumption 
of lepton universality in the SM~\cite{Aaij:2015yra}.
By profiting of the large dataset collected during Run-I, the LHCb experiment is expected 
...
be lowered selecting candidates with good-quality tracks and vertices.
...
%
As new physics is expected not to affect tree level $b\to\cquark\cquarkbar\squark$ processes the ratio 
between the \jpsi channels, $r_{\jpsi}$, is 1 and therefore \mbox{$\RKst^{'} = \RKst \cdot r_{\jpsi} = \RKst$}.
On the other hand using the relative efficiencies between the rare and resonant channels
allows to cancel out many systematic effects resulting in a better control of systematic uncertainties.  
...
The choice of the lower limit of the low-\qsq interval is driven by the need to reject the background due to the $\Bz\to\Kstarz\gamma$
...
from $\phi\to\ll$ decays, that can dilute new physics effects, while the upper bound 
is chosen to be sufficiently far away from the \jpsi radiative
...
conditions and are combined in the right proportions, according to the luminosities registered on data.
