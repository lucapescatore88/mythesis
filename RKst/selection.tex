...
\caption{Two-dimensional \qsq \emph{vs} $m(K\pi\ell\ell)$ distributions for 
...
\caption{Correct ID \emph{vs} mis-ID probability distributions for 
...
pass the selection requirements. Figure~\ref{fig:phiplots} (left) shows a plot of $m(K\pi)$ \emph{vs}
...
magnitude larger than \BdToKstee and it may pass the selection requirements when the two neutrinos have low momentum. 
...
when the photon converts into a \ee\! pair, it has similar characteristics to \BdKstee. 
...
For the electron rare decay channels the vertex constraint alone is not sufficient to remove all background.
...
\caption{Two-dimensional distributions of $\log(\chisq_{\rm FD})$ \emph{vs} \mbcm for (left) 
...
(bottom) Signal efficiency \emph{vs} background rejection.
...
This number is then scaled by the ratio between the rare and resonant branching fractions and
