...
still contain both ID combinations for most candidates, therefore tighter PID requirements are applied.
...
Figure \ref{fig:e_mu_pid} shows correct ID probability distributions 
for electrons and muons in data, while Fig.~\ref{fig:k_pi_pid} shows the probabilities of correct 
...
The same requirements are applied to the muon and electron channels, unless specified differently.
...
The ratio between the \pt of the \Kstarz and the dielectron pair, $\alpha = \pt(\Kstarz) / \pt(\ee)$, can be used to check this hypothesis.
...
The the neural network input consists of 24 variables carrying information about the kinematic of the decays
