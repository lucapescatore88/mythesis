...
In order to select the $\Bz \to\Kstar(\jpsi\to \mumu)$ channel a dilepton mass interval of 100 \mevcc
...
\multirow{4}{*}{ \Kstar }
       			& $\pt > 500$ \mevc \\
       			& $|m - m_{\Kstar}^{PDG}| < 100$ \mevcc  \\ %300 in stripping but then we restrict it to 100
       			& $\chisqip(primary) > 9$ \\
...
Track-quality and vertex quality cuts are also applied using the $\chi^2_{track}/\text{ndf}$, {\verb GhostProb }, and $\chi^2_{vtx}/\text{ndf}$
...
A large mass window is kept around the \Bz peak in order to be able
...
using the ``{\verb ProbNN }" variables \cite{ProbNNs_pres}. These variables are the output of a Neural Network which
takes as input information from the calorimeters, the RICH detectors and the muon system.
...
\section{Breamsstrahlung recovery for electrons}
...
particle ($1/m^2$) and therefore is relevant only for electrons. At these energies, if electrons radiate after
...
desirable to recover these bremsstrahlung photons, when possible.
A tool for bremsstrahlung recovery is available in the LHCb analysis software.
This tool looks for other clusters in the calorimeter and reconstructing the trajectory of the electron
checks if they may have been emitted by that. Then the photon energy is added to the electron and its momentum
recalculated. Figure \ref{bremreco} shows a schematic view of the process. For more information see
...
\subsection{Cascade $b \rightarrow c\ell$, $c \rightarrow \ell$ decays}
...
above 5150 \mevcc. %, which is sufficient to fit the peak.
...
%Id provides index to correlation matrices shown in Fig.~\ref{fig:Rkst_nnCorrelation}.
