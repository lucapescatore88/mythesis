...
Table~\ref{tab:candidates} summarises the requirements to distinguish sample from different channels.
...
\multirow{4}{*}{$\mu\mu$}
	...
	& \BdToKstJPsmm (\mcKpimm)	& $|m_{\mm} - m_{\jpsi}^{PDG}| < 100\mevcc$ \\ \hline
\multirow{7}{*}{$ee$} 
...
\begin{tabular}{$c|^c}
\rowstyle{\bfseries}
\boldmath{$\mu\mu$} candidates &  \boldmath{$ee$} candidates \\
\hline
	L0Muon		& L0Electron\\
				& L0Hadron\\
				& L0Global (TIS)\\
...
\begin{table}[]
\begin{center}
\caption{Summary of stripping requirements. }
\begin{tabular}{$c^c}
\rowstyle{\bfseries}
...
\begin{figure}[h!]
\centering 
\includegraphics[width=0.49\textwidth]{RKst/figs/RKst/phi.pdf}
\includegraphics[width=0.49\textwidth]{RKst/figs/RKst/Kmumu.pdf}
...
\caption{Distribution of (left) $\cos (\theta_\ell)$ and of (right) the $m(\Kstarz e)$ invariant mass, where the \decay{\Bd}{(\Dm\to K \en \overline{\nu})\ep\nu} background is selected by requiring $m(\Kstarz\epem) < 4800\mevcc$. The red distribution corresponds to events with $| \cos ( \theta_\ell ) | > 0.8$.}
\label{fig:Denu_background}
\end{figure}

In the muon channels the background from \decay{\Bd}{(\Dm\to K \mun \overline{\nu})\mup\nu} decays is suppressed by the choice of the fitting range.
...
i%s used to rescale the prediction of the LHCb MC to get rid of problems related to an absolute prediction. 
...
$h$		& $min,max(p_{\textrm{T}, \kaon},p_{\textrm{T}, \pi})$, \quad $min,max(\chi_{\textrm{IP}, \kaon}^{2},\chi_{\textrm{IP}, \pi}^{2})$ \\
$\ell\ell$	& \pt, \quad \chisqip, \quad $\chisq_{\rm FD}$, \quad $\chisq_{vtx}/\text{ndf}$, \quad \texttt{DIRA} \\
$\ell$	& $min,max(p_{\textrm{T}, \ell^+},p_{\textrm{T}, \ell^-})$, \quad $min,max(\chi_{\textrm{IP}, \ell^+}^{2},\chi_{\textrm{IP}, \ell^-}^{2})$ \\
...
The number of signal events accepted by a given requirement is determined using a data-driven method.
