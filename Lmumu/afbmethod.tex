...
%$d\Gamma_X^{mm'}/d q^2=X^{mm'}$ and $X^{V+A}=X^{V}+X^{A}$.
...
contributions and $S$ the scalar contribution. The apices $V$ and $A$ represent respectively
...
%\frac{d\Gamma}{dq^2}\afbl\cos\theta_\ell+
...
%\includegraphics[width=0.49\textwidth]{Lmumu/figs/resolution/RmTcosThetaL_vs_q2_LL.pdf}
 \caption{Difference between generated and reconstructed angular observables, $\cos\theta_\ell$ (left) and
 $\cos\theta_h$ (right) as a function of 
 the observables themselves for long (top) and downstream (bottom) candidates. 
