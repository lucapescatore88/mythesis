...
that the \Lb vertex is at least $\sqrt{n}$ standard deviations away from the primary vertex.
Another useful quantity used to remove combinatorial background is a pointing variable
defined as the angle between the direction of the particle momentum and
the flight direction from its mother vertex, called DIRA. This allows the selection of particles with well-defined primary vertices.
...
\end{figure}
%
The variable $\chi^2_{FD}$ represents the flight distance with respect to the origin vertex divided by its uncertainty. 
...
Loose PID requirements are also applied in pre-selection to limit the size of the samples.
...
Finally, specific trigger lines are selected, corresponding to events triggered by the muons
