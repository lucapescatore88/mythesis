...
\rowstyle{\bfseries}
Model   		& \boldmath{$\chi^2/ndf$}  & \boldmath{$NDF$}  & p-value  & \boldmath{$N_{evts}$} \\ \hline
...
\rowstyle{\bfseries}
\boldmath{\qsq [\gevgevcccc]} & Sig. PDF bias (\%)  & Bkg. PDF bias (\%)  & Tot. sys. (\%) \\ \hline
...
New physics could affect the decay model by adding contributions to the $C_7$ and $C_9$ 
...
The \Lb lifetime is known with limited precision. For evaluation of the efficiencies the
...
The simulation used to calculate the efficiency is re-weighted as described in Sec.~\ref{sec:kinWeight}.
...
uncorrelated uncertainty ($\sigma_{uncorr}^{k}$), together with relative values of the correlated uncertainty ($\sigma_{corr}$). }
...
Figure~\ref{fig:corrDDLLplots} shows the branching fraction obtained by fitting 
...
the relative values listed in Tab.~\ref{tab:Lb_combBR} by the absolute branching fraction of the normalisation channel,
...
an enhanced yield is expected due to the proximity of the photon pole and in region between the two charmonium 
...
but lies below the predictions in the low-\qsq region.
