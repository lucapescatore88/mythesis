...
This gives a solid basis to the theory introducing a mechanism that produces the particles masses~\cite{Susskind:1978ms}.
...
\begin{table}[h!]
\centering
\caption{Predicted and measured values of the decay widths of the \Z and \W bosons and the anomalous
...
SM results from the application of the principle of invariance under the unitary product group 
...
to interact with each other. First of all we can distinguish between fermions (half-integer spin particles) and 
bosons (integer spin particles). Fermions constitute the basic building blocks of matter, while bosons are 
the mediators of the interaction between them. Since in the SM the concept of bosonic mediators of 
interactions arises because of gauge symmetry~\cite{Glashow:1961tr},
%
\begin{table}[h!]
...
\end{table}
%
they are called ``gauge bosons". The list of the known interactions with their force carrier and properties 
is reported in Tab.~\ref{tab:interactions}. The matter of which we are made is mainly composed of electrons and protons, which have spin 1/2; protons are then composed of \uquark and \dquark quarks, which again have 
...
\end{figure}
%
In Eq.~\ref{CKM} is reported a parametrisation of the CKM matrix together with the most
...
\begin{figure}[h!]
\centering 
\includegraphics[width=0.55\textwidth]{Introduction/figs/Unitarity_triangle.png}
...
theory diverges. On the other hand gravity becomes irrelevant for small masses as those of particles and can be neglected in good approximation.
...
at an energy scale $\mu$. The first part, dealing with short distance physics handles with
...
The classic example of an effective theory is the Fermi theory of weak interactions, illustrated in Fig.~\ref{fig:fermi_theory}, which describes the $\beta$ decay in terms of a four-fermion interaction
where the short distance physics is hidden into a point like vertex.
\begin{figure}[h!]
\centering
...
method of the Operator Product Expansion (OPE)~\cite{Buchalla:1995vs} was used.
...
Coefficients, $C_i$. Operators and coefficient are evaluated at the renormalization scale $\mu$.
...
\end{equation}
%
where $\alpha_s$ is the strong coupling constant. In the SM at $\mu_W = m_b$ the
...
The operators are that are relevant for radiative transitions and leptonic and hadronic penguin processes 
...
and the $\mathcal{O}_8$ by a gluon. The $\mathcal{O}_7$ operator is the dominant contribution to the radiative 
...
are treated theoretically in different ways. 
%
...
by working in the heavy quark limit, $b\to\infty$. In this limit a Heavy Quark Effective Theory (HQET)~\cite{DellaMorte:2015yda} 
can be constructed in which the heavy quark interacts only via `soft' hadronic processes and 
an OPE in $1/m_b$ is valid.
...
The boundary of this region can be set at $\sim 7$ \gev~ as this corresponds to the threshold for 
$\cquark\cquarkbar$ production, $(2m_c)^2 \sim 7$ \gevgevcccc. 
...
The intermediate region is characterised by the presence of charmonium resonances, though the tree 
...
Although the decays can be experimentally vetoed in
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.6\textwidth]{Introduction/figs/P5prime.png}
