...
and electroweak (EW) interactions. It was formulated in his current form in the mid-70s and has 
been an extremely successful and predictive theory since then. Almost all known phenomena 
from 1~\ev~up to several hundred \gev~are well 
...
together in atoms and its mediator is the photon.
%
...
by the emission or absorption of $W^\pm$ and $Z$ bosons. Unlike the electromagnetic force,
...
In 1968 Salam, Glashow and Weinberg unified the weak and electromagnetic forces in a single theory, where the coupling
constants of the electromagnetic, $e$, and weak, $g$, interactions are linked by the weak mixing angle, $\theta_W$
by the relation $g\sin\theta_W = e$~\cite{PDG2014}. 
%
The electroweak symmetry is spontaneously broken by the Higgs
field~\cite{Strocchi:1977za} and this causes the $W^\pm$ and \Z bosons to become massive (see Tab.~\ref{tab:interactions}) 
...
space that a particle can cover in a time $\Delta t$ is $r = c \Delta t$, qualitatively $r \sim \hbar / mc$. In this picture
the carriers of the weak force can travel $r \sim 2 \cdot 10^{-3}$~fm. The photon must instead be massless in the theory, 
...
loops. The second group of EW interactions, NC, corresponds to diagrams mediated by a photon or a \Z boson interacting
with a fermion and its anti-fermion.
...
was motivated in order to explain why some decays, although kinematically allowed, had never been observed. To all leptons is
...
been observed. This is explained by the fact that the lepton number in the initial
...
Considering a 6 quark system one angle is not enough to describe a rotation but the mixing can be generalised
...
can be divided in 3 real constants and one imaginary phase. The imaginary phase generates the \mbox{CP-violation} which was
...
\end{figure}
%
Equation~\ref{CKM} reports a parametrisation of the CKM matrix together with the most
recent measured values of its elements~\cite{PDG2014}. In this parametrisation $\rho$, $A$, and $\lambda$
are the real constants and $\eta$ the imaginary phase and Eq.~\ref{params} shows their relations with the 3 mixing angles.
%
\begin{multline}
...
imbalance. However, the magnitude of CP violation predicted by the SM, $O(10^{-20})$, is not enough 
...
zero mass~\cite{Maltoni:2011zz,Cleveland:1998nv,Fukuda:1998mi,Eguchi:2002dm}.
%
...
the SM (BSM). Among the most promising approaches there are those involving Super-Symmetry and extra-dimensions. 
%
...
sign to the mass term, these would naturally cancel out~\cite{Fayet:1976cr}. Supersymmetry also provides a candidate for dark
...
one can consider simplified models, where the SM is extended incorporating a new sector
...
by the need to explain existing tensions between data and SM predictions or by theoretical prejudice.
%
...
%
Leptoquarks are bosonic particles that carry both quark and lepton flavour quantum numbers.
For simplicity they are commonly assumed to be scalar particles.
...
can occur trough loop diagrams such as penguin or \W box diagrams (see Fig.~\ref{fig:penguins}).
...
new particles could be at high mass scales not accessible at colliders but their effect
...
a great quantity of observables that can be affected by new physics, not only decay rates, but also 
...
which are the main topic of this thesis, the doubly Cabibbo-suppressed contributions proportional to $\lambda^u_s$ 
can be neglected. To obtain this formula the Operator Product Expansion (OPE)~\cite{Buchalla:1995vs} method
...
\mbox{(e.g. $I = B$ and $F=\Kstarz\mumu$)} the decay amplitude can be calculated as
%
\begin{equation}
A(I\to F)= \langle I | \mathcal{H}_{eff} | F \rangle = 
= \frac{G_F}{\sqrt{2}} \sum V_{CKM}^i C_i(\mu) \langle I | \mathcal{O}_i(\mu) | F \rangle,
\end{equation}
...
\end{table}
%
The \mbox{``low $q^2$"} is instead the region where the light spectator quark is energetic and cannot be neglected.
...
An other way to build safe observables is to construct ratios between similar
...
It is also interesting to compare decays which go though the same fundamental process but
...
To set the background for the analysis described in this thesis, this section reports a 
...
Decays of $B$ mesons into a dimuon are two-body decays where the two muons are back to
...
\end{align}
%
Both decays were previously unobserved, while now the $\Bs$ decay is observed
...
At the LHC energies is now possible to collect large data samples of semileptonic 
...
new physics, particular interest was risen by the measurement of a set of observables in
...
a local $3.7\sigma$ deviation. Attempts to build a consistent picture point to a new physics contribution
...
$\mathcal{A}_{CP}$ measurements are a powerful null test of the SM. 
...
3~\invfb, measured both these asymmetries to be consistent with zero~\cite{LHCB-PAPER-2014-006,Aaij:2014bsa},
as reported in Tab.~\ref{tab:AcpAI}. 
%
\begin{table}
\begin{small}
...
ratios in the SM that can be enhances by new physics. They are therefore a natural place to look for new physics.
