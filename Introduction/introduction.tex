...
\begin{table}[h!]
\centering
\caption{Predicted and measured values of the decay widths of the \Z and \W bosons~\cite{PDG2014}.
...
$SU(3)_C €‰\otimes ‰SU(2)_{W} €‰\otimes €‰U(1)_Y$, which reflects conservation laws such as the conservation 
...
Weak		& $Z$, $W^\pm$	& $10^{-16}$		& $10^{-18}$	& $W^\pm = 80.399$~\gevcc \\
			&		&			&		& $Z_0 = 91.188$~\gevcc	\\
...
the doublets participating in the weak interactions are an admixture of the mass eigenstates. 
...
\end{equation}  
Therefore a $3 \times 3$ matrix depends then on 4 real parameters, which
...
\end{figure}
%
Equation~\ref{CKM} reports the most recent measured values of its elements~\cite{PDG2014}
together with the Wolfenstein parametrisation which highlights the hierarchical structure of the matrix.
...
and Eq.~\ref{params} shows their relations with the 3 mixing angles; terms further from the diagonal 
...
\caption{Feynman diagrams of (left) neutral current allowed in the SM, where$f$ represents any fermion,
and (center-right) FCNCs processes forbidden in the SM.}
...
new sources of FCNCs but also allow for natural explanations of the equality of the proton and electron charges.
On the other hand the observation of neutrino oscillation proves that flavour is not always conserved
suggesting flavour structures beyond the SM. Furthermore, the values of the terms of the CKM matrix and the PMNS
...
%
The idea to introduce extra-dimensions was triggered by the fact that, normally, gravity is not relevant in particle physics but it would 
...
as an intermediate model building step. Instead of constructing thories valid up to the GUT scale
one can consider simplified models, where the SM is extended by the addition of a new sector
...
for simplicity are commonly assumed to be scalar particles.
...
The large masses of the W and Z bosons and top quark compared to that of the \bquark quark allow
...
ton pole
at very low \qsq, charmonium resonances at central \qsq and broad resonances at high \qsq~\cite{TomRDreview}.}
...
these measurements is often limited by uncertainties on the perturbative part of the calculations.
...
At the LHC energies is possible to collect large data samples of semileptonic 
...
\multirow{2}{*}{Asymmetry}	& \multicolumn{2}{c|}{$\Bz\to K^+\mumu$}			&\multicolumn{2}{c}{$\Bz\to \Kstarz\mumu$}	\\ \cline{2-5}
...
\end{table}
%
Recently, progress was also made measuring also electron channels.
